...
if SERVER then
				...
				if vFireIsCharacter(ent) and vFireEnableSpread then
					...
					if ent:IsPlayer() and math.random(1, 4) == 1 or !ent:IsPlayer() then
						...
						if newFeed > 0 then
							...
							local spreadChance = self:SpreadPlayer(ent)
				...
				if ran < 575 then -- Attempt to spread upwards
					...
					if tr.Hit and tr.Fraction > 0 then
						...
						if newFeed > 0 then
							...
							-- if didSpread then
								-- print("Fire has spreaded from " .. self.parent:GetClass() .. " to " .. ent:GetClass() .. ". / Chance: " .. math.Round(spreadChance * 100))
							-- else
								-- print("Fire did not spread from " .. self.parent:GetClass() .. " to " .. ent:GetClass() .. ". / Chance: " .. math.Round(spreadChance * 100))
							-- end
						end
					end
	...
	function ENT:SpreadPlayer(ent)
		local chance = 1
		if ent:IsPlayer() then
			chance = vFirePCSpreadBase
			if self.parent == ent then
				chance = chance * vFirePCSpreadToSelf
			end
			if ent:IsOnFire() then
				chance = chance * vFirePCSpreadOnFire
			end
			-- print("Spread Base: " .. vFirePCSpreadBase)
			-- print("Spread OnFire: " .. vFirePCSpreadOnFire)
			-- print("Spread Self: " .. vFirePCSpreadToSelf)
		end
		return chance
	end
end
